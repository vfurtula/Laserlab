		...
		for step_pos in self.arr_smc100:
			...
			for set_position in self.arr_cm110:
				...
				while (time.time()-time_s)<self.dwell_time:
					...
					if self.inst_list.get("SR530"):
						bit_step = -1
						Vout = self.inst_list.get("SR530").return_voltage(1)
...
class Run_CM110(QMainWindow):
	...
	def initUI(self):
		...
		g1_2.addWidget(lcd_cm100_lbl,0,4)
		...
		g1_2.addWidget(self.lcd_cm100,1,4)
		v1 = QVBoxLayout()
		v1.addLayout(g1_0)
		v1.addLayout(g1_1)
		v1.addLayout(g1_2)
	...
	def set_run(self):
		...
		if self.inst_list.get("SMC100"):
			if self.startst_Edit.text() and not self.stopst_Edit.text() and not self.stepst_Edit.text():
				try:
					self.arr_smc100 = numpy.arange(float(self.startst_Edit.text()),float(self.startst_Edit.text())+1,1)
				except Exception as e:
				...
				try:
					self.arr_smc100 = numpy.arange(float(self.startst_Edit.text()),float(self.stopst_Edit.text())+float(self.stepst_Edit.text()),float(self.stepst_Edit.text()))
				except Exception as e:
		...
		self.timer.stop()
		self.get_thread=Run_CM110_Thread(self.arr_cm110,dwelltime,dwell_ard,self.avg_pts,file_txt,file_hdf5,self.timestr,self.analogref,self.inst_list,self.arr_smc100,self.cb_cm110,self.cb_smc100)
	...
	def save_(self):
		...
		self.config.set(self.last_used_scan,"start", str(self.startEdit.text()) )
		self.config.set(self.last_used_scan,"stop", str(self.stopEdit.text()) )
		self.config.set(self.last_used_scan,"step", str(self.stepEdit.text())  )
